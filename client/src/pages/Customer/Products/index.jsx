...
export default function Products() {
    ...
    const handleAddToCart = async (storeProductId) => {
        const response = await dispatch(
            createCartItemAction({ storeProduct: storeProductId, quantity: 1 })
        );
        if (response?.error) {
    ...
    const handleAddFavoriteStoreProduct = async (storeProductId) => {
        const response = await dispatch(
            addFavoriteStoreProductAction(storeProductId)
        );
        if (response?.error) {
    ...
    const handleRemoveFavoriteStoreProduct = async (storeProductId) => {
        const response = await dispatch(
            deleteFavoriteStoreProductAction(storeProductId)
        );
        if (response?.error) {
            ...
            <Grid container spacing={2}>
                {productData?.products?.map((product) => (
                    <Grid item xs={6} md={3} key={product?._id}>
                        <Card
                            className="product_card-primary"
                            key={product?._id}
                        >
                            <Box>
                                <Swiper
                                    ...
                                    pagination={true}
                                    modules={[PaginationSwipper]}
                                >
                                    {product?.productId?.images?.map(
                                        (image) => (
                                            <SwiperSlide
                                                key={image?._id}
                                    ...
                                    {favoriteData?.products?.some(
                                        ...
                                        >
                                            <Button
                                                ...
                                                onClick={() =>
                                                    handleRemoveFavoriteStoreProduct(
                                                        product?._id
                                                    )
                                                }
                                            >
