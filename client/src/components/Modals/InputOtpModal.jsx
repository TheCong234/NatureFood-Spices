    ...
    >
      <Box sx={style}>
        ...
        <Box sx={{ display: sendOTP ? "block" : "none" }}>
          <p className={`text-green-500 ${sendOtpStatus ? "block" : "hidden"}`}>
            Gửi OTP thành công tới email đã đăng ký của bạn, vui lòng nhập mã và
            "Xác thực"
          </p>
          <p className={`text-red-500 ${sendOtpStatus ? "hidden" : "block"}`}>
            Gặp vấn đề khi Gửi OTP, vui lòng kiểm tra email và thử lại
          </p>
        </Box>
        <Box textAlign={"end"}>
          <Button
